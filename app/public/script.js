...
fetch('/api/config')
    ...
    .catch(err => {
        console.warn('Não foi possível carregar configuração do backend, usando URL padrão');
    });
...
function getBotResponse(message) {
    ...
    if (lowerMessage.includes('olá') || lowerMessage.includes('oi') || lowerMessage.includes('bom dia') || lowerMessage.includes('boa tarde') || lowerMessage.includes('boa noite')) {
        return 'Olá! Bem-vindo à Hello. Posso tirar dúvidas sobre nossos planos de telefonia móvel, coberturas, ativação de chip ou qualquer outro assunto.';
    }
